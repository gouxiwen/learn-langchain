...
# LangChain 的 AIMessage（AI 消息）标准结构主要关注两件事：
# content：最终回复的文本。
# tool_calls：是否调用了工具。
# 但对于 DeepSeek Reasoner，我们还需要一个额外的字段 reasoning_content 来保留推理过程中的中间内容。
class DeepSeekReasonerChatModel(BaseChatModel):
    ...
    def __init__(self, **kwargs):
        ...
        if OpenAI is None:
            raise ImportError("需要安装 openai 库。请运行: pip install openai")
