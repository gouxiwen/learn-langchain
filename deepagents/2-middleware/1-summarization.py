...
# 配置摘要中间件，并创建一个包含该中间件的代理。
# model：指定用于生成摘要的模型。摘要的质量直接取决于此模型的能力。
# trigger：设置触发摘要的条件。例如 ("messages", 5) 表示当消息列表中的消息数量大于 5 条时，触发摘要。
# keep：设置摘要后需要保留的最新消息数量。例如 ("messages", 3) 表示摘要完成后，只保留最近的 3 条原始消息，其余被压缩成一条摘要。
agent = create_agent(
    model=model,
    tools=[internet_search, calculate],
    middleware=[
        SummarizationMiddleware(
            model=model, # 这里可以选个小模型，比如gpt-3.5-turbo
            trigger=("messages", 5),
            keep=("messages", 3),
        ),
    ],
...
# 测试调用与效果验证：笔者构造一个包含四条历史消息的初始状态，然后追加一条新的用户询问，最后调用 Agent 进行处理。
# 从返回的结果可以看到，Agent 的回复中包含了五条消息，其中第一条 HumanMessage 正是 Summarization 中间件对历史信息进行总结后生成的摘要内容。
status = {
    "messages":[
